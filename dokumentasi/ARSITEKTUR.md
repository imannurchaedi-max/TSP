...
- **Tab "Input Reservasi"**: Fitur copy-paste terpusat bagi operator TSP untuk menyalin blok data reservasi mentah dari SAP. 
    - Textarea akan di-parse otomatis untuk mengekstrak data MID dan Quantity.
    - Quantity dengan format `1.000,50` (koma desimal Eropa) dikonversi ke `1000.50`.
    - Auto-lookup ke Material Master untuk mendapatkan *Deskripsi* dan *UOM*.
    - Pengiriman bulk (batch) records yang secara langsung di-*inject* (suntik) ke Spreadsheet **Ecosystem WRM** (bukan ditulis ke sheet lokal TSP). Data yang di-*inject* ini pada akhirnya akan kembali tersinkronisasi ke TSP melalui `IMPORTRANGE` pada tab `BARCODE OUTBOUND WRM` untuk dikonsumsi oleh fitur Penerimaan.
...
| v116-v121 | *(tidak terdokumentasi)* - pakai `git log` untuk menelusuri, bukan tabel ini. |
| v122 | Promosi ulang tanpa perubahan kode saat verifikasi setup clasp (9 September 2026). Isi `@122` identik byte-per-byte dengan `@121`; dinaikkan hanya untuk memvalidasi rantai `npm run deploy` yang kini menyertakan langkah prune. Tidak ada perubahan perilaku produksi. |
