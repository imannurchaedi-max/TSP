...
> **Wajib:** jangan jalankan `flutter build apk --release` langsung dari folder
> SynologyDrive. Folder ini memakai reparse point; file simbol native Gradle dapat
> terbaca bukan sebagai file normal dan membuat build release gagal.
...
Launcher tersebut otomatis mencerminkan source ke `C:\BuildWorkspaces\TSPModul`,
menjalankan `flutter clean`, `flutter pub get`, `flutter analyze`, `flutter test`, dan
build release di disk lokal. Hanya APK final yang disalin kembali ke SynologyDrive.
...
`ApiClient` memakai resolver jaringan bawaan Android/Dart sehingga dapat memakai IPv4 atau IPv6
sesuai jaringan. Jangan mengunci IPv4: Google Apps Script berada di endpoint Google dual-stack,
...
2. Apps Script balas redirect (**umumnya HTTP 302**) → app membaca `Location` dan melakukan
   **GET tanpa body** ke URL echo satu-pakai itu. Dio tidak boleh mengandalkan
   `Response.isRedirect`; status redirect ditangani eksplisit.
...
| `native_symbol_tables` / `libapp.so.sym: not a regular file` | Jangan hapus artefak SynologyDrive secara manual. Jalankan `BUILD_RELEASE_LOCAL.cmd`; build berlangsung di `C:\BuildWorkspaces\TSPModul`. |
